...
PRAVO_MINISTRIES = [
    ("foiv266", "Минсельхоз"),
    ("foiv090", "Минприроды"),
    ("foiv264", "Минвостокразвития"),
    ("foiv345", "ФАДН"),
    ("foiv270", "Росрыболовство"),
    ("foiv096", "Рослесхоз"),
]

# Корни слов — ловят все падежи
PRAVO_ROOTS = [
    "коренн",
    "кмнс",
    "малочисленн",
...
def is_relevant_pravo(text):
    t = text.lower()
    return any(root in t for root in PRAVO_ROOTS)
...
def parse_pravo(keywords, days_back=30):
    ...
    def process_page(soup, ministry_name):
        items = []
        for a_tag in soup.select("a[href*='/document/']"):
            title = a_tag.get_text(strip=True)
            if not title or len(title) < 15:
                continue
            if not is_relevant_pravo(title):
                continue
            href = a_tag.get("href", "")
    ...
    for ministry_code, ministry_name in PRAVO_MINISTRIES:
        ...
        results += process_page(soup, ministry_name)
        time.sleep(1.0)
...
def parse_regulation(keywords, days_back=60):
    ...
    for kw in keywords[:6]:
        try:
            r = requests.post(
                search_url,
                json={"Name": kw, "SortColumn": "PublicationDate",
                      "SortAscending": False, "PageSize": 20, "Page": 1, "StatusId": None},
...
def parse_fadn(days_back=30):
    ...
    for url, section_type in sections:
        ...
        soup = BeautifulSoup(r.text, "html.parser")
        candidates = (
            soup.select("article.news") or soup.select(".news-item") or
            soup.select(".content-list li") or soup.select("div.item") or
...
def save_digest(results, path, source_status=None, new_only=True):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        if source_status:
            ...
            if failed:
                f.write(f"  Недоступны: {', '.join(failed)}\n")
        f.write("═" * 70 + "\n\n")
        sources = [
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="КМНС Монитор v1.4")

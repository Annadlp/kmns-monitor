...
def handle_command(token, chat_id, text, config):
    ...
    send_message(token, chat_id,
        ...
        send_message(token, chat_id,
            "📜 <b>pravo.gov.ru</b> — принятые НПА\n\n"
            "🏛 <b>sozd.duma.gov.ru</b> — законопроекты в Думе\n\n"
            "📋 <b>regulation.gov.ru</b> — проекты до Думы (можно подать замечания)\n\n"
            "🏢 <b>fadn.gov.ru</b> — новости ФАДН")
    else:
...
def scheduled_check(token, chat_id, config):
    ...
    new_docs = [r for r in results if r.get("новый")]
    if new_docs:
        send_message(token, chat_id, format_new_docs(new_docs))
